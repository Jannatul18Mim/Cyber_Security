...
echo "[+] Generating and patching config.inc.php..."
cd /var/www/html/DVWA/config
cp config.inc.php.dist config.inc.php

# Automating the credential edit using sed
# Note: These match the SQL user created in step 3
sed -i "s/'db_user'     ] = 'dbuser';/'db_user'     ] = 'admin';/g" config.inc.php
sed -i "s/'db_password' ] = 'dbpass';/'db_password' ] = 'password';/g" config.inc.php

# 3. Database Initialization
echo "[+] Starting MySQL and creating 'dvwa' database..."
...
echo "[*] Access Lab: http://localhost/DVWA/setup.php"
echo "[*] Instructions: Click 'Create / Reset Database' to finish."
echo "-------------------------------------------------------"
